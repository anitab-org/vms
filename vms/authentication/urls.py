...
urlpatterns = [
    ...
    url(r'^logout/$',
     ...
     url(r'^password_reset/$',
        PasswordResetView.as_view(success_url='done/',form_class=EmailValidationOnForgotPassword),
        name='password_reset'),
    url(r'^password_reset/done/$',
        PasswordResetDoneView.as_view(template_name='registration/password_reset_done.html'),
        name='password_reset_done'),
    url(r'^reset/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$',
        PasswordResetConfirmView.as_view(success_url='/authentication/reset/complete/'), name='password_reset_confirm'),
